...
namespace cowsins
{
    public class Crosshair : MonoBehaviour
    {
        ...
        [Header("Variables")]
        [SerializeField, Tooltip("If enabled, the crosshair will not be displayed when the game is paused.")]
        private bool hideCrosshairOnPaused;

        [SerializeField, Tooltip("If enabled, the crosshair will not be displayed when the player is inspecting.")]
        private bool hideCrosshairOnInspecting;
        ...
        [Header("Hitmarker")] [SerializeField] private bool hitmarker;
        ...
        private void Update()
        {
            ...
            if (inputActions.Player.Firing.IsPressed() && player.weaponController != null &&
                player.weaponController.canShoot)
                if (spread != defaultSpread)
                    spread = Mathf.Lerp(spread, defaultSpread,
            ...
            if (player.Grounded)
            {
                ...
                else
                {
                    ...
                    {
                        Resize(rb.linearVelocity.magnitude < .2f ? defaultSpread : walkSpread);
                    }
        ...
        public void SpotEnemy(bool condition)
        {
            ...
            width = (condition)
                ? Mathf.Lerp(width, enemySpottedWidth, resizeSpeed)
                : Mathf.Lerp(width, originalWidth, resizeSpeed);
        }
        ...
        void OnGUI()
        {
            if (playerStats.isDead
                || weaponController.weapon != null && weaponController.isAiming &&
                player.weaponController.removeCrosshairOnAiming || !isVisible) return;
            ...
            if (crosshairShape.parts.downPart)
                GUI.DrawTexture(
                    new Rect(Screen.width / 2 - width / 2, (Screen.height / 2 - size / 2) + spread / 2, width, size),
                    texture);

            if (crosshairShape.parts.topPart)
                GUI.DrawTexture(
                    new Rect(Screen.width / 2 - width / 2, (Screen.height / 2 - size / 2) - spread / 2, width, size),
                    texture);

            if (crosshairShape.parts.rightPart)
                GUI.DrawTexture(
                    new Rect((Screen.width / 2 - size / 2) + spread / 2, Screen.height / 2 - width / 2, size, width),
                    texture);

            if (crosshairShape.parts.leftPart)
                GUI.DrawTexture(
                    new Rect((Screen.width / 2 - size / 2) - spread / 2, Screen.height / 2 - width / 2, size, width),
                    texture);

            if (!crosshairShape.parts.center) return;
            var radius = Mathf.Min(width, size) / 2;
