        ...
        private void Update()
        {
            if (!player.Grounded)
            {
                ...
                transform.localPosition = Vector3.Lerp(transform.localPosition,
                    new Vector3(0, jumpMotion.Evaluate(motion), 0) * distance, motion);
                transform.localRotation = Quaternion.Lerp(transform.localRotation,
                    Quaternion.Euler(new Vector3(jumpMotion.Evaluate(motion) * rotationAmount, 0, 0)), motion);
            }
            else
            {
                motion = 0;
                ...
                transform.localPosition = Vector3.Lerp(transform.localPosition,
                    new Vector3(0, jumpMotion.Evaluate(motion2), 0) * distance, motion2);
                transform.localRotation =
                    Quaternion.Lerp(transform.localRotation, Quaternion.Euler(Vector3.zero), motion2);
            }
        }
    }
}