...
namespace cowsins
{
    public class ProceduralShot : MonoBehaviour
    {
        ...
        private static ProceduralShot _instance;

        public static ProceduralShot Instance
        {
        ...
        private void Update()
        {
            ...
            var aimingTransl = weapon.isAiming && pattern != null ? pattern.aimingTranslationMultiplier : 1;
            var aimingRot = weapon.isAiming && pattern != null ? pattern.aimingRotationMultiplier : 1;

            // Apply the motions
            transform.localPosition = new Vector3(x * pattern.translationDistance.x, y * pattern.translationDistance.y,
                z * pattern.translationDistance.z) * aimingTransl;
            transform.localRotation = Quaternion.Euler(new Vector3(xRot * pattern.rotationDistance.x,
                yRot * pattern.rotationDistance.y, zRot * pattern.rotationDistance.z) * aimingRot);
        }
